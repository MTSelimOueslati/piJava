...
date1.setValue(dateTime);
 ...
 if(localDate.compareTo(date1.getValue())>0 )
        ...
        Optional<ButtonType> action = alert.showAndWait();
                       ...
                       String MSG = " l'évenement  " + titre1.getText() + "  qui est une  " + combo.getValue() + "  est consacré pour " + date1.getValue().toString()  ;
...
pb.setVisible(false);
  ...
  @FXML 
    ...
    @FXML
    private void parti(ActionEvent event) {
                               FXMLLoader loader = new FXMLLoader
                        (getClass()
                         .getResource("AdminPartic.fxml"));
            try {
                Parent root = loader.load();         
                Scene Donia = new Scene(root,1200,800);
